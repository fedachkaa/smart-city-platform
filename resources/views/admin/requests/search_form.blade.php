<div class="bg-white shadow-lg rounded-lg mb-8 p-6">
    <form method="GET" action="{{ route('dashboard.requests.index') }}" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
                <label for="title" class="block text-sm font-medium text-gray-700">Title</label>
                <input type="text" name="title" id="title" value="{{ request('title') }}" class="p-2  mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm">
            </div>
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('dashboard.requests.index') }}" class="py-2 px-4 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                    Clear
                </a>
            </div>
        </div>
