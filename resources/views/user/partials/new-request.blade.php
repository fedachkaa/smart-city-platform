<div class="min-h-screen bg-gray-100 p-6">
    <div class="max-w-3xl mx-auto bg-white shadow-2xl rounded-xl p-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('messages.profile.new_request.title') }}</h2>

        <form method="POST" action="{{ route('profile.requests.store') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf

            <div class="relative group">
                <label for="title" class="block text-sm font-medium text-gray-700">{{ __('messages.profile.new_request.title_field') }}</label>
            ...
            <div class="relative group">
                <label for="description" class="block text-sm font-medium text-gray-700">{{ __('messages.profile.new_request.description') }}</label>
            ...
            <div class="relative group">
                <label for="photo" class="block text-sm font-medium text-gray-700">{{ __('messages.profile.new_request.photo') }}</label>
            ...
            <div class="pt-2">
                <button type="submit"
                    ...
                    {{ __('messages.profile.new_request.submit') }}
                </button>
            </div>
        </form>
    </div>
